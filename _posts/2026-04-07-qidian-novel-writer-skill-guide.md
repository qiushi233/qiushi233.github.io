...
author: latte
categories: [AI, 创作工具]
tags: [网文写作, AI写作, 起点中文网, 番茄小说, CatDesk, Skill]
toc: true
---
...
```
~/.catpaw/skills/qidian-novel-writer/novels/<书名>/
...
这个技能是 CatDesk AI 助手的专属 Skill，安装后即可在对话中直接使用。如果你也在使用 CatDesk，可以通过技能市场搜索 `qidian-novel-writer` 安装。
...
/继续      → 继续撰写下一章节
/保存章节  → 将当前章节内容保存到本地 skill 目录（见第十九节）
/剧情分析  → 分析当前已写章节的情节发展和质量
/故事总结  → 总结已写章节的主要事件、角色发展和主题
/冲突      → 执行冲突检查，分析和优化故事冲突
/指令      → 显示完整指令集和用法说明
...
1. **本地保存**：将章节内容写入 `~/.catpaw/skills/qidian-novel-writer/novels/<书名>/chapter-NN.md`，并更新 `setting.md` 的「已发布章节」表格（见第十九节）
...
> 工具：`~/.catpaw/bin/catdesk browser-action`
...
// 标题（可直接用 fill selector）
// browser-action fill: input[placeholder="请输入标题"]
```
...
```bash
# browser-action key 命令
browser-action key: Meta+s
...
// 触发 Cmd+A 全选后粘贴（通过 browser-action key 命令）
// browser-action key: Meta+a
// browser-action key: Meta+v
```
...
**批量发布脚本思路（Python + browser-action）：**
...
for i, ch in enumerate(chapters):
    ...
    # 3. 导航到发布页（通过 browser-action navigate）
...
```bash
mkdir -p ~/.catpaw/skills/qidian-novel-writer/novels/<书名>
```
...
输出：「✅ 已保存：`~/.catpaw/skills/qidian-novel-writer/novels/<书名>/chapter-NN.md`」
